...
import { IU2UProvider } from '@iu2u/sdk'; // Hypothetical SDK

// Initialize providers
const provider = new ethers.providers.Web3Provider(window.ethereum);
const signer = provider.getSigner();

// IU2U contract instances
const iu2uAddresses = {
  4157: '0x...', // CrossFi
  1: '0x...',    // Ethereum
  56: '0x...',   // BSC
...
// Initialize IU2U instance
const chainId = await provider.getNetwork().then(n => n.chainId);
const iu2u = new ethers.Contract(iu2uAddresses[chainId], iu2uABI, signer);
```
...
async function sendCrossChainTokens() {
  try {
    ...
    const amount = ethers.utils.parseEther('10'); // 10 IU2U
    
    // Check user balance
    const balance = await iu2u.balanceOf(await signer.getAddress());
    if (balance.lt(amount)) {
      throw new Error('Insufficient IU2U balance');
    }
...
async function callCrossChainContract() {
  try {
    ...
    const tx = await iu2u.callContract(
      destinationChain,
      contractAddress,
      payload
    );
...
function IU2UWallet() {
  ...
  async function initializeWallet() {
    try {
      if (window.ethereum) {
        ...
        // Get IU2U balance
        const iu2uBalance = await iu2u.balanceOf(address);
        setBalance(ethers.utils.formatEther(iu2uBalance));
  ...
  return (
    <div className="iu2u-wallet">
      <h2>IU2U Wallet</h2>
      
      <div className="balances">
        <div>IU2U Balance: {balance} IU2U</div>
...
contract CrossChainDApp is IU2UExecutable {
    struct Message {
        string content;
        address sender;
        string sourceChain;
    ...
    function _executeWithToken(
        ...
        require(
            keccak256(bytes(symbol)) == keccak256(bytes("IU2U")),
            "Unsupported token"
        );
    ...
    function withdrawTokens(uint256 amount) external {
        ...
        // Transfer IU2U tokens
        IERC20 iu2u = IERC20(gateway.tokenAddresses("IU2U"));
        require(iu2u.transfer(msg.sender, amount), "Transfer failed");
    }
...
contract CrossChainSender {
    IIU2UGateway public immutable iu2uGateway;
    IERC20 public immutable iu2uToken;
    
    constructor(address gateway_, address iu2uToken_) {
        iu2uGateway = IIU2UGateway(gateway_);
    ...
    function sendMessageWithTokens(
        ...
        require(
            iu2uToken.transferFrom(msg.sender, address(this), tokenAmount),
            "Token transfer failed"
        );
        
        // Approve gateway to spend tokens
        require(
            iu2uToken.approve(address(iu2uGateway), tokenAmount),
            "Approval failed"
        );
...
### Hypothetical IU2U SDK

```javascript
// @iu2u/sdk - Hypothetical SDK implementation
class IU2UProvider {
  constructor(config) {
    this.config = config;
    this.providers = {};
  ...
  async deposit(amount, chain = 'crossfi') {
    const iu2u = await this.getContract(chain);
    const tx = await iu2u.deposit({
  ...
  async withdraw(amount, chain = 'crossfi') {
    const iu2u = await this.getContract(chain);
    const tx = await iu2u.withdraw(ethers.utils.parseEther(amount));
    return await tx.wait();
  }
  
  async sendCrossChain(destinationChain, recipient, amount) {
    const iu2u = await this.getContract('crossfi'); // Always send from CrossFi
    const tx = await iu2u.sendToken(
      destinationChain,
      recipient,
      'IU2U',
  ...
  async callCrossChain(destinationChain, contractAddress, payload, tokenAmount = null) {
    const iu2u = await this.getContract(); // Current chain
    
    if (tokenAmount) {
      const tx = await iu2u.callContractWithToken(
        destinationChain,
        contractAddress,
        payload,
        'IU2U',
...
async function crossChainLend() {
  const lendingPayload = ethers.utils.defaultAbiCoder.encode(
    ['uint256', 'address'],
    [ethers.utils.parseEther('100'), userAddress] // Lend 100 IU2U
  );
...
async function handleIU2UTransaction(transactionFn) {
  try {
...
async function safeTransfer() {
  try {
    const result = await handleIU2UTransaction(async () => {
      return await iu2u.sendCrossChain('ethereum', recipient, amount);
    });
    
    console.log('Transfer successful:', result.transactionHash);
  } catch (error) {
    if (error instanceof IU2UError) {
      console.error(`IU2U Error [${error.code}]:`, error.message);
      console.error('Details:', error.details);
      
      // Handle specific error codes
      switch (error.code) {
        case 'INSUFFICIENT_BALANCE':
          alert('Please deposit more IU2U tokens before transferring');
          break;
...
This integration guide provides comprehensive examples for developers to integrate IU2U into their applications, covering frontend integration, smart contract development, API usage, and error handling patterns.
