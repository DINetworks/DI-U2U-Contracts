...
async function main() {
    ...
    // 1. Deploy IU2U Token (if not already deployed)
    let iu2uToken;
    try {
        const IU2U = await ethers.getContractFactory("IU2U");
        iu2uToken = await IU2U.deploy(
            "IU2U Token",
            "IU2U",
            18,
        ...
        console.log("⚠️  IU2U Token deployment failed or already exists");
    }
    ...
    // Mint some IU2U tokens to aggregator for testing
    if (iu2uToken) {
        await iu2uToken.mint(aggregator.address, ethers.utils.parseEther("1000000"));
        console.log("✅ Minted 1M IU2U tokens to aggregator");
    }
    ...
    if (iu2uToken) console.log(`🪙 IU2U Token: ${iu2uToken.address}`);
    ...
    console.log("=" * 50);
    if (iu2uToken) {
        console.log(`npx hardhat verify --network ${network.name} ${iu2uToken.address} "IU2U Token" "IU2U" 18 "1000000000000000000000000000"`);
    }
    console.log(`npx hardhat verify --network ${network.name} ${calldataGenerator.address} "${deployer.address}"`);
    console.log(`npx hardhat verify --network ${network.name} ${aggregator.address} "${mockGateway}" "${mockGasService}" "${iu2uToken?.address || 'IU2U_ADDRESS'}" "${calldataGenerator.address}"`);
    ...
    const deploymentConfig = {
        ...
        contracts: {
            iu2uToken: iu2uToken?.address || null,
    ...
    console.log("4. Test cross-chain swaps on U2U testnet");
    console.log("5. Audit contracts before mainnet deployment");

    return {
        iu2uToken: iu2uToken?.address,
        calldataGenerator: calldataGenerator.address,
        crossChainAggregator: aggregator.address,
        deploymentConfig
    };
...
async function testCrossChainSwap() {
    ...
    // Example swap: 100 USDC on Ethereum → IU2U → 100 USDT on BSC
    const swapRequest = {
