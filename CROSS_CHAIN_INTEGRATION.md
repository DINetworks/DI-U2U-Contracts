...
This document provides comprehensive examples for integrating and using the IU2U Cross-Chain Aggregator system.
...
# Verify contracts
npx hardhat verify --network ethereum AGGREGATOR_ADDRESS "GATEWAY" "GAS_SERVICE" "IU2U_TOKEN" "CALLDATA_GENERATOR"
```
...
describe("CrossChainAggregator", function () {
    let aggregator, iu2uToken, calldataGenerator;
    ...
    beforeEach(async function () {
        ...
        iu2uToken = await IU2U.deploy("IU2U", "IU2U", 18, ethers.utils.parseEther("1000000"));
        ...
        await iu2uToken.mint(aggregator.address, ethers.utils.parseEther("1000000"));
    });

    describe("Cross-Chain Swap", function () {
        it("Should initiate cross-chain swap successfully", async function () {
            const swapData = {
                sourceToken: iu2uToken.address,
            ...
            // Mint tokens to user
            await iu2uToken.mint(user1.address, ethers.utils.parseEther("1000"));
            await iu2uToken.connect(user1).approve(aggregator.address, swapData.sourceAmount);
        ...
        it("Should handle token execution correctly", async function () {
            const payload = ethers.utils.defaultAbiCoder.encode(
                ...
                [user1.address, ethers.utils.parseEther("100"), iu2uToken.address]
            );
    ...
    describe("Calldata Generation", function () {
        it("Should generate Uniswap V2 calldata correctly", async function () {
            const calldata = await calldataGenerator.generateUniswapV2Calldata(
                ...
                [iu2uToken.address, "0xA0b86a33E6441c45C74d7F7f5234f3628B8b5C22"],
                user1.address,
...
class SystemHealthMonitor {
    ...
    async checkTokenBalances() {
        // Check IU2U token balances in aggregator contracts
        for (const [chainId, provider] of this.providers) {
            const iu2uToken = new ethers.Contract(
                this.contracts.iu2uToken[chainId],
                ERC20_ABI,
                provider
            );

            const balance = await iu2uToken.balanceOf(
                this.contracts.aggregator[chainId]
            );

            const minBalance = ethers.utils.parseEther("10000"); // 10k IU2U minimum
            
            if (balance.lt(minBalance)) {
                throw new Error(`Low IU2U balance on chain ${chainId}: ${ethers.utils.formatEther(balance)}`);
            }
        }
    }
...
This comprehensive integration guide provides everything needed to deploy, integrate, and maintain the IU2U Cross-Chain Aggregator system in production environments.
