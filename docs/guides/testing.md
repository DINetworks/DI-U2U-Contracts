...
```javascript
// test/unit/IU2U.test.js
const { expect } = require("chai");
const { ethers } = require("hardhat");
const { loadFixture } = require("@nomicfoundation/hardhat-network-helpers");

describe("IU2U Token", function () {
  async function deployIU2UFixture() {
    const [owner, addr1, addr2] = await ethers.getSigners();

    const IU2U = await ethers.getContractFactory("IU2U");
    const iu2u = await IU2U.deploy();

    return { iu2u, owner, addr1, addr2 };
  }

  describe("Deployment", function () {
    it("Should set the right name and symbol", async function () {
      const { iu2u } = await loadFixture(deployIU2UFixture);

      expect(await iu2u.name()).to.equal("IU2U Protocol");
      expect(await iu2u.symbol()).to.equal("IU2U");
    });

    it("Should set the right decimals", async function () {
      const { iu2u } = await loadFixture(deployIU2UFixture);
      expect(await iu2u.decimals()).to.equal(18);
    });

    it("Should assign the total supply to the owner", async function () {
      const { iu2u, owner } = await loadFixture(deployIU2UFixture);
      
      const ownerBalance = await iu2u.balanceOf(owner.address);
      const totalSupply = await iu2u.totalSupply();
  ...
  describe("Transfers", function () {
    it("Should transfer tokens between accounts", async function () {
      const { iu2u, owner, addr1, addr2 } = await loadFixture(deployIU2UFixture);

      // Transfer 50 tokens from owner to addr1
      await expect(iu2u.transfer(addr1.address, 50))
        .to.changeTokenBalances(iu2u, [owner, addr1], [-50, 50]);

      // Transfer 50 tokens from addr1 to addr2
      await expect(iu2u.connect(addr1).transfer(addr2.address, 50))
        .to.changeTokenBalances(iu2u, [addr1, addr2], [-50, 50]);
    });

    it("Should fail if sender doesn't have enough tokens", async function () {
      const { iu2u, owner, addr1 } = await loadFixture(deployIU2UFixture);
      const initialOwnerBalance = await iu2u.balanceOf(owner.address);

      await expect(
        iu2u.connect(addr1).transfer(owner.address, 1)
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");

      expect(await iu2u.balanceOf(owner.address)).to.equal(initialOwnerBalance);
    });
  ...
  describe("Allowances", function () {
    it("Should approve and transfer from", async function () {
      ...
      await iu2u.approve(addr1.address, 100);
      expect(await iu2u.allowance(owner.address, addr1.address)).to.equal(100);

      // Transfer 50 tokens from owner to addr2 via addr1
      await expect(iu2u.connect(addr1).transferFrom(owner.address, addr2.address, 50))
        .to.changeTokenBalances(iu2u, [owner, addr2], [-50, 50]);

      // Check remaining allowance
      expect(await iu2u.allowance(owner.address, addr1.address)).to.equal(50);
    });
    ...
    it("Should fail transferFrom without allowance", async function () {
      ...
      await expect(
        iu2u.connect(addr1).transferFrom(owner.address, addr2.address, 50)
