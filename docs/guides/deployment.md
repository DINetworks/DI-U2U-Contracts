...
git clone https://github.com/iu2u/iu2u-contracts.git
cd iu2u-contracts
...
async function main() {
  ...
  // 2. Deploy IU2U Token
  console.log("\n2. Deploying IU2U Token...");
  const IU2U = await ethers.getContractFactory("IU2U");
  const iu2uToken = await IU2U.deploy();
  await iu2uToken.deployed();
  
  console.log("IU2U Token deployed to:", iu2uToken.address);
  deploymentData.contracts.iu2uToken = iu2uToken.address;
  ...
  // 6. Deploy IU2U Executable (for cross-chain calls)
  console.log("\n6. Deploying IU2U Executable...");
  const IU2UExecutable = await ethers.getContractFactory("IU2UExecutable");
  const IU2UExecutable = await IU2UExecutable.deploy(
    networkConfig.axelarGateway,
    networkConfig.axelarGasService,
    aggregator.address
  );
  await IU2UExecutable.deployed();
  
  console.log("IU2U Executable deployed to:", IU2UExecutable.address);
  deploymentData.contracts.IU2UExecutable = IU2UExecutable.address;
  ...
  // Set aggregator in IU2U executable
  await IU2UExecutable.setAggregator(aggregator.address);
  console.log("✓ Aggregator set in IU2U executable");
...
async function main() {
  ...
  const verifications = [
    ...
    {
      name: "IU2UExecutable",
      address: contracts.IU2UExecutable,
      constructorArguments: [
...
async function main() {
  ...
  await gasCreditVault.setConversionRate(ethers.utils.parseUnits("2000", 18)); // 1 IU2U = 2000 gas units
...
describe("Deployment Integration Tests", function () {
  ...
  before(async function () {
    ...
    // Get contract instances
    contracts.iu2uToken = await ethers.getContractAt("IU2U", deploymentData.contracts.iu2uToken);
  ...
  describe("Contract Deployment", function () {
    it("Should have deployed all contracts", function () {
      expect(contracts.iu2uToken.address).to.be.properAddress;
  ...
  describe("Basic Functionality", function () {
    ...
    it("Should be able to deposit gas credits", async function () {
      ...
      await contracts.iu2uToken.connect(user1).approve(contracts.gasCreditVault.address, depositAmount);
