...
```html
<!-- Core IU2U SDK -->
<script src="https://cdn.iu2u.com/sdk/latest/iu2u-sdk.min.js"></script>

<!-- Ethers.js (required dependency) -->
<script src="https://cdn.ethers.io/lib/ethers-5.7.2.umd.min.js"></script>

<script>
// IU2U SDK is now available globally
const { IU2UGateway, CrossChainAggregator } = IU2U;
</script>
```
...
```javascript
import { IU2UGateway, CrossChainAggregator, MetaTxGateway } from '@iu2u/sdk';
...
const config = {
  ...
  gasOracleEndpoint: 'https://gas.iu2u.com'
};

const gateway = new IU2UGateway(config);
...
class WalletManager {
  ...
  async connectCoinbaseWallet() {
    ...
    const coinbaseWallet = new CoinbaseWalletSDK({
      appName: 'IU2U DApp',
      appLogoUrl: 'https://iu2u.com/logo.png',
      darkMode: false
    });
...
// Context for IU2U SDK
const IU2UContext = createContext();

export const IU2UProvider = ({ children, config }) => {
  ...
  useEffect(() => {
    const initializeSDK = async () => {
      try {
        const gatewayInstance = new IU2UGateway(config);
  ...
  return (
    <IU2UContext.Provider value={{ gateway, aggregator, isInitialized }}>
      {children}
    </IU2UContext.Provider>
  );
};

export const useIU2U = () => {
  const context = useContext(IU2UContext);
  if (!context) {
    throw new Error('useIU2U must be used within IU2UProvider');
  }
  return context;
};
...
export const useIU2U = () => {
  const initialize = async (config) => {
    try {
      state.config = config;
      state.gateway = new IU2UGateway(config);
...
class IU2UErrorHandler {
  ...
  handleError(error, context = '') {
    console.error(`[IU2U] Error in ${context}:`, error);
...
const errorHandler = new IU2UErrorHandler();
...
```javascript
class IU2UPerformanceOptimizer {
  constructor() {
    this.cache = new Map();
...
```javascript
// tests/iu2u-integration.test.js
import { IU2UGateway, CrossChainAggregator } from '@iu2u/sdk';
import { ethers } from 'ethers';

describe('IU2U Integration', () => {
  let gateway;
  let aggregator;
  let mockProvider;
  let mockSigner;
  ...
  beforeEach(() => {
    ...
    gateway = new IU2UGateway(config);
...
export const productionConfig = {
  ...
  relayer: {
    endpoint: 'https://relayer.iu2u.com',
    timeout: 30000
  },
  features: {
...
- [React Example App](https://github.com/iu2u/react-example)
- [Vue Example App](https://github.com/iu2u/vue-example) 
- [JavaScript SDK Documentation](https://docs.iu2u.com/sdk)
