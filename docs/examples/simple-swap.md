...
This example demonstrates how to perform basic token swaps using IU2U Protocol, including single-chain swaps with various DEX aggregators.

## Overview

Token swaps are the fundamental operation in DeFi, allowing users to exchange one token for another. IU2U Protocol aggregates liquidity from multiple DEXs to provide optimal swap rates.
...
import { IU2UGateway, TokenSwap } from '@iu2u/sdk';
...
contract SimpleSwapExample is ReentrancyGuard {
    ...
    constructor(address _iu2uGateway) {
        iu2uGateway = IIU2UGateway(_iu2uGateway);
    }
    ...
    function executeSwap(
        ...
        if (tokenIn == address(0)) {
            ...
            IERC20(tokenIn).approve(address(iu2uGateway), amountIn);
        }

        // Prepare swap parameters
        IIU2UGateway.SwapParams memory params = IIU2UGateway.SwapParams({
            tokenIn: tokenIn,
            tokenOut: tokenOut,
            amountIn: amountIn,
        ...
        // Execute swap
        amountOut = iu2uGateway.executeSwap{value: msg.value}(params);
    ...
    function executeBatchSwaps(
        IIU2UGateway.SwapParams[] calldata swaps
        ...
        // Execute each swap
        for (uint i = 0; i < swaps.length; i++) {
            IIU2UGateway.SwapParams memory swap = swaps[i];
            
            // Handle token transfers
            if (swap.tokenIn != address(0)) {
                IERC20(swap.tokenIn).transferFrom(msg.sender, address(this), swap.amountIn);
                IERC20(swap.tokenIn).approve(address(iu2uGateway), swap.amountIn);
            }

            // Execute swap
            uint256 ethValue = swap.tokenIn == address(0) ? swap.amountIn : 0;
            amountsOut[i] = iu2uGateway.executeSwap{value: ethValue}(swap);
    ...
    function findOptimalRoute(
        ...
        for (uint i = 0; i < routerTypes.length; i++) {
            try iu2uGateway.getQuote(tokenIn, tokenOut, amountIn, routerTypes[i]) 
    ...
    function _executeSingleSwap(
        ...
        amountOut = iu2uGateway.executeSwap{value: ethValue}(params);
    }
    ...
    function getSwapQuote(
        ...
        return iu2uGateway.getQuote(tokenIn, tokenOut, amountIn, routerType);
    }
...
- [API Reference](../api-reference/iu2u-gateway.md)
