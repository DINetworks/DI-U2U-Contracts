...
contract DataReceiver is IU2UExecutable {
...
```solidity
contract ConditionalExecutor is IU2UExecutable {
...
```solidity
contract CallbackContract is IU2UExecutable {
...
```solidity
contract MultiChainState is IU2UExecutable {
...
```solidity
contract CrossChainGovernance is IU2UExecutable {
    struct Proposal {
        string description;
...
```solidity
contract RobustCaller is IU2UExecutable {
    struct FailedCall {
        string targetChain;
        address targetContract;
        bytes payload;
...
contract ReentrancyProtectedCaller is ReentrancyGuard, IU2UExecutable {
    function safeCall(
        string memory targetChain,
        address targetContract,
        bytes memory payload
...
```solidity
contract MockGateway is IIU2UGateway {
    ...
    function _simulateExecution(string memory contractAddress, bytes memory payload) internal {
        ...
        if (target != address(0)) {
            try IU2UExecutable(target).execute(
                keccak256("mock"),
                "source_chain",
...
- [IU2U Gateway API](../api-reference/iu2u-gateway.md)
