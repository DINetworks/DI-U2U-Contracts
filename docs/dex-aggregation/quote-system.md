...
import { IU2UQuoteSystem } from '@iu2u/sdk';

export function useIU2UQuote(request, options = {}) {
    const [quote, setQuote] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [alternatives, setAlternatives] = useState([]);

    const quoteSystem = new IU2UQuoteSystem(options.config);
...
```javascript
const { IU2UQuoteSystem } = require('@iu2u/sdk');

class TradingBot {
    constructor(config) {
        this.quoteSystem = new IU2UQuoteSystem(config);
